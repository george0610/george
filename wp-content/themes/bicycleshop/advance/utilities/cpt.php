<?php
if(!function_exists( 'bicycleshop_cpt_meta_fields' )){
	/**
	 * Return internet address and custom fields of post
	 *
	 * @return string
	 */
    function bicycleshop_cpt_meta_fields(){
        $fields = '';
	    $custom_fields_number = apply_filters('bicycleshop_custom_fields_number', 5);
        ...
        for($i = 0; $i <= $custom_fields_number; $i++){
	        if( $i === 0 ) {
		        ...
		        if ( strlen( $temp ) ) {
			        $temp = esc_html__( 'Website', 'bicycleshop' ).':'.$temp;
		        }
	        }
	        else{
		        //custom fields
		...
		if(strlen($fields)){
			$html =
				'<div class="meta-data">'
			       ...
			       .apply_filters('bicycleshop_after_cpt_meta_fields','').
		        '</div>';
		}
...
if(!function_exists('bicycleshop_make_media_collection')){
    ...
    function bicycleshop_make_media_collection($id = null, $args = array()){
	    ...
	    <ul class="gallery-media-collection screen-reader-text">
			<?php echo bicycleshop_prepare_frontend_gallery_html( bicycleshop_prepare_gallery_attachments( $value, $id ), $order, $id, $args ); ?>
		</ul>
	    <?php
    }
}
 ...
 * @return array
 */
function bicycleshop_prepare_gallery_attachments( $value, $post_id = null ){
	global $bicycleshop_a13;
	...
	if ( ! empty( $value ) ) {
		...
		$proofing_enabled = $bicycleshop_a13->bicycleshop_get_meta( '_proofing', $post_id ) === 'on';
...
function bicycleshop_frontend_gallery_item_html( $collector, $post_id, $args ){
	...
	<li class="<?php echo esc_attr($collector[ 'item_class' ]); ?>"<?php echo $data_attr;//escaped while creating above ?>>
		...
		if( $collector[ 'attachment_type' ] === 'internal' && $collector[ 'type' ] === 'video' ){
			...
			echo '<div class="album-video" id="album-video-'.esc_attr( $id ).'">'.bicycleshop_video( $video_attr ).'</div>';
		}
 ...
 */
function bicycleshop_calculate_height_proportion($brick_proportion){
	$height_proportion = 0;
	//prepare proportion from string
 ...
 */
function bicycleshop_get_media_filters($id = null){
