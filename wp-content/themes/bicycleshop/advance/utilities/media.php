...
if(!function_exists('bicycleshop_get_top_image_video')) {
	/**
	 ...
	 */
	function bicycleshop_get_top_image_video( $link_it = false, $args = '' ) {
        ...
        if(
            ($is_post && $bicycleshop_a13->get_option( 'post_media') == 'off')
            ||
            ($is_post_list && $bicycleshop_a13->get_option( 'blog_media') == 'off')
        ){
            return $html; //empty string
        }
        ...
        $image_video    = $bicycleshop_a13->get_option( 'blog_videos') === 'off' && ($is_post_list || $is_shortcode);

        $thumb_size = 'skt-post-thumb'; //default for post

        if( ! $is_shortcode && ( $is_page || $is_post || $is_work ) ){
            $layout = $is_page? $bicycleshop_a13->bicycleshop_get_meta('_content_layout', $post_id) : $bicycleshop_a13->get_option( 'post_content_layout');
            $full_layouts =  array(
                'full_padding',
                'full',
            );
            ...
            elseif(in_array($layout, $small_layouts)){
                ...
                if( defined('BICYCLESHOP_NO_SIDEBARS') || $bicycleshop_a13->bicycleshop_get_meta( '_widget_area' ) == 'off'){
                    $thumb_size = 'skt-post-thumb-big';
                }
            }
        }
        elseif( $is_post_list || $is_shortcode ){
            if($bicycleshop_a13->get_option( 'blog_post_look') === 'horizontal'){
                $thumb_size = 'skt-blog-posts-horizontal';
            }
            else{
                $thumb_size = 'skt-blog';
                $brick_size         = $bicycleshop_a13->bicycleshop_get_meta('_brick_ratio_x', $post_id);
                $columns            = array_key_exists( 'columns', $args ) ? $args['columns'] : (int)$bicycleshop_a13->get_option( 'blog_brick_columns' );
                $bricks_max_width   = array_key_exists( 'max_width', $args ) ? $args['max_width'] : (int)$bicycleshop_a13->get_option( 'blog_bricks_max_width' );
                $brick_margin       = array_key_exists( 'margin', $args ) ? $args['margin'] : (int)$bicycleshop_a13->get_option( 'blog_bricks_margin' );
                ...
                if( $bicycleshop_a13->bicycleshop_get_meta( '_image_parallax' ) !== 'on' ){
                    $image_proportion =  $bicycleshop_a13->get_option( 'blog_bricks_proportions_size', '0' );
                    $height_proportion =  bicycleshop_calculate_height_proportion($image_proportion);
        ...
        if( $args['force_image'] || $img_or_vid === 'post_image' ){
            $is_parallax = $bicycleshop_a13->bicycleshop_get_meta( '_image_parallax' ) === 'on';
            if($args['return_src']){
                $html = bicycleshop_make_post_image($post_id, $sizes[$thumb_size], true);
            }
            elseif($is_parallax){
	            $img_src = bicycleshop_make_post_image($post_id, $sizes[$thumb_size], true);
                if($img_src !== false ){
                    $parallax_height = $bicycleshop_a13->bicycleshop_get_meta('_image_parallax_height', $post_id);
                ...
                $additional_class = '';
                $src = bicycleshop_make_post_image( $post_id, $sizes['full'], true );
                ...
                if( $file_type['type'] === 'image/gif'){
                    $img = bicycleshop_make_post_image($post_id, $sizes['full']);
                    $additional_class = ' animated-gif';
                }
                else{
                    $img = bicycleshop_make_post_image($post_id, $sizes[$thumb_size]);
                }
        ...
        elseif( $img_or_vid === 'post_video' ){
            ...
            if($image_video){
	            $html = bicycleshop_get_top_image_video($link_it, array_merge($args, array('force_image' => true )));
            }
            else{
                ...
                if( !empty( $src ) ){
                    ...
                    if($v_code === false){
                        $video_data  = bicycleshop_detect_movie($src);
...
if(!function_exists('bicycleshop_top_image_video')){
    /**
     ...
     *
     */
	function bicycleshop_top_image_video($link_it = false, $args = ''){
        //bicycleshop_get_top_image_video() produces escaped content
        echo bicycleshop_get_top_image_video($link_it, $args);
    }
}


if(!function_exists('bicycleshop_make_post_image')){
	/**
	 ...
	 */
	function bicycleshop_make_post_image( $post_id, $sizes, $only_src = false ){
        global $bicycleshop_a13;
...
if(!function_exists('bicycleshop_detect_movie')){
	/**
	 ...
	 */
	function bicycleshop_detect_movie($src){
...
if(!function_exists('bicycleshop_get_movie_link')){
	/**
	 * Returns movie link to for video API
	 *
	 * @param array $video_data data received from bicycleshop_detect_movie function
	 *
	 * @return bool|string API URL if video type is matched, false otherwise
	 */
	function bicycleshop_get_movie_link( $video_data ){
 ...
 */
function bicycleshop_video( $attr, $dont_load_video_library = false ) {
	global $content_width, $bicycleshop_a13;
	$post_id = 0;
 ...
 * @return float ration
 */
function bicycleshop_get_svg_height_ratio($svg){
