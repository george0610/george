...
add_action( 'wp_enqueue_scripts', 'bicycleshop_theme_scripts', 26 ); //put it later then woocommerce
if(!function_exists('bicycleshop_theme_scripts')){
	/**
	 * Registering frontend theme scripts
	 */
	function bicycleshop_theme_scripts(){
        ...
        $script_depends = array( 'bicycleshop-plugins' );
        ...
        wp_register_script('bicycleshop-plugins', get_theme_file_uri( 'js/helpers.min.js' ),
            false, //depends
            BICYCLESHOP_THEME_VERSION, //version number
            true //in footer
        );
	    ...
	    $script_depends[] = 'bicycleshop-isotope';

        //flickity needed
        $flickity_themes = array('scroller', 'scroller-parallax');

		//A13 STICKY ONE PAGE
		wp_register_script( 'jquery-slimscroll', get_theme_file_uri( 'js/jquery.slimscroll.min.js' ), array('jquery','bicycleshop-plugins'), '1.3.2', true);
		wp_register_script( 'fullPage', get_theme_file_uri( 'js/jquery.fullPage.min.js' ), array('jquery','jquery-slimscroll'), '2.7.6', true);
		if( $bicycleshop_a13->bicycleshop_get_meta( '_content_sticky_one_page' ) === 'on' ){
...
if(!function_exists('bicycleshop_js_parameters')){
	...
	function bicycleshop_js_parameters(){
        ...
        if($proofing){
            ...
            $params['proofing_comment_placeholder'] = esc_html__( 'Write your comment here&hellip;', 'bicycleshop' );
...
// ]]>
        ...
        global $bicycleshop_a13;
        $use_loader = $bicycleshop_a13->get_option( 'use_webfontloader', 'on' ) === 'on';
...
add_action( 'wp_enqueue_scripts', 'bicycleshop_get_web_fonts_static' );
if(!function_exists( 'bicycleshop_get_web_fonts_static' )) {
    function bicycleshop_get_web_fonts_static(){
        global $bicycleshop_a13;
        $use_loader = $bicycleshop_a13->get_option( 'use_webfontloader', 'on' ) === 'on';
